...
{
    Dictionary<PlaceType, HashSet<Vector2Int>> 
        tileByType = new Dictionary<PlaceType, HashSet<Vector2Int>>();
        ...
        HashSet<Vector2Int> roomFloorNoCorridor)
    {
        ...
        foreach (var position in roomFloorNoCorridor)
        {
            ...
            PlaceType type = neighboursCount8Dir < 8 ? PlaceType.NearWall : PlaceType.OpenSpace;

            if (tileByType.ContainsKey(type) == false)
                tileByType[type] = new HashSet<Vector2Int>();

            if (type == PlaceType.NearWall && graph.GetNeighbours4Directions(position).Count == 4)
                continue;
            tileByType[type].Add(position);
        }
    }

    public Vector2? GetItemPlacementPosition(PlaceType placementType, int iterationsMax, Vector2Int size, bool addOffset)
    {
        ...
        while (iteration < iterationsMax)
        {
            ...
            if (itemArea > 1)
            {
                ...
                tileByType[PlaceType.NearWall].ExceptWith(placementPositions);
            }
            else
            {
